...
class RAGSystem:
    ...
    def _process_excel_file(self, file_path: str) -> List[Document]:
        ...
        try:
            ...
            for sheet_name in xls.sheet_names:
                ...
                print(
                    f"📋 Processing sheet: {sheet_name} with {len(df)} rows and {len(df.columns)} columns"
                )
        ...
        if UNSTRUCTURED_AVAILABLE:
            try:
                print(
                    f"📊 Using Unstructured fallback for Excel processing: {file_path}"
                )
                ...
                if documents:
                    print(
                        f"📊 Unstructured processed {len(documents)} documents from Excel"
                    )
                    return documents
    ...
    def _process_pdf_file(self, file_path: str) -> List[Document]:
        ...
        if self.llama_parser:
            try:
                ...
                print(
                    f"⚠️ LlamaParse failed, falling back to PyPDFLoader: {llama_error}"
                )
        ...
        if UNSTRUCTURED_AVAILABLE:
            try:
                ...
                if documents:
                    print(
                        f"📄 Unstructured processed {len(documents)} documents from PDF"
                    )
                    return documents
    ...
    def query(
        ...
        try:
            ...
            print(
                f"🔍 Query analysis: {query_analysis.intent} intent, {query_analysis.language} language, confidence: {query_analysis.confidence:.2f}"
            )
